...
class AppController(
    ...
    fun apps(@InputArgument page: Int, @InputArgument size: Int): AppPage {
        return withAdmin {
            appService.getAll(page, size).let { (contents, pageInfo) ->
                AppPage(
                    contents.map {
                        App(
                            it.id.toString(),
    ...
    @DgsQuery
    fun app(@InputArgument id: String): App {
        return withAdmin {
            appService.getDtoById(id).toGraphQL()
        }
    }
    ...
    fun activateApp(@InputArgument input: ActivateAppInput): App {
        return withAdmin {
            ...
            appService.getDtoById(input.id).toGraphQL()
        }
    }
    @DgsMutation
