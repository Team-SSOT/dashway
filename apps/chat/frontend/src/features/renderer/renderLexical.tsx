...
function walkNode(
  ...
  switch (node.type) {
    ...
    case 'mention': {
      ...
      const memberId = mentionNode.memberId ?? mentionNode.id ?? ''
      const label = mentionNode.label ?? mentionNode.text ?? memberId
      const displayName = opts.membersById?.[memberId]?.name
      return (
        <MentionRender
          key={key}
          memberId={memberId}
